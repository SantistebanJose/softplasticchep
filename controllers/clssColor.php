...
function guardarColor()
{
    ...
    if ($id === 0) {
        $result = executeQuery($conectar, "
            INSERT INTO color (nombre, descripcion, rgb, created_at)
            VALUES (:nombre, :descripcion, :rgb, NOW())
            RETURNING id
        ", [
            'nombre'      => $nombre,
...
function eliminarColor()
{
    ...
    executeQuery(
        $conectar,
        "UPDATE color SET deleted_at = NOW(), update_at = NOW() WHERE id = :id",
        ['id' => $id]
    );
...
function reactivarColor()
{
    ...
    executeQuery(
        $conectar,
        "UPDATE color SET deleted_at = NULL, update_at = NOW() WHERE id = :id",
        ['id' => $id]
    );
